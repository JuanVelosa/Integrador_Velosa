package com.example.integradorvelosabackend.util;
...
public class DFTUtils {
    ...
    public static SignalTransformation fourierTransformation(List<Test> fullTests) {
        double[] signal = new double[fullTests.size()];
        for (int i = 0; i < fullTests.size(); i++) {
            double signalTransformation = Math.sqrt(Math.pow(fullTests.get(i).getxPosition(), 2)
                    + Math.pow(fullTests.get(i).getyPosition(), 2) + Math.pow(fullTests.get(i).getzPosition(), 2));
            signal[i] = signalTransformation;
        }
        double[] normalizedSignal = DFTUtils.normalize(signal);
        double[] spectrum = DFTUtils.dftSpectrum(normalizedSignal);
        double[] freqs = DFTUtils.dftFreq(normalizedSignal, 0.5);
        return new SignalTransformation(spectrum, freqs);
    }
